...
smote_input <- train_data %>% select(-time)
...
train_X <- train_data %>% select(-status, -time)
...
X_train <- train_data_smote %>% select(-status, -time)
...
X_test <- test_data %>% select(colnames(X_train))
